...
> <strong>注意：</strong>你需要处理日出日落或月升月落返回为空的情况，避免造成你的程序故障。
...
> <strong>提示：</strong>南北半球观看到的月相形状是左右倒置的，但名称是一样的。在数据中的`moonPhase.icon`已经进行了适配。例如当处于蛾眉月的时候，北半球的`moonPhase.icon = 801`，而南半球的`moonPhase.icon = 807`
