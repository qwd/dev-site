---
title: Configuration
description: Before you can start using QWeather Developers Service, you need to do
  some basic configuration, such as build an API request or setup the SDK and etc.
url: "/en/docs/configuration/"
translationKey: config-overview
---

Before you can start using QWeather Developers Service, you need to do some basic configuration, such as build an API request or setup the SDK and etc.

- [Project and Credential](/en/docs/configuration/project-and-key/)
- [API Host](/en/docs/configuration/api-host/)
- [Authentication](/en/docs/configuration/authentication/)
- [API Config](/en/docs/configuration/api-config/)
- [iOS SDK Config](/en/docs/configuration/ios-sdk-config/)
- [Android SDK Config](/en/docs/configuration/android-sdk-config/)
