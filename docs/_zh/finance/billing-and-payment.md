...
```
300000次请求量 x 0.001元 + 700000次请求量 x 0.00088元 = 916元
```
...
- 账单金额：669.6元（0.001元 x 300000次请求 + 0.00088元 x 420000次请求）
- 应付金额：669.6元
- 账单状态：待支付
 
此时你的可用额度为-569.6元（100元可用额度 - 669.6元应付金额）。你可以在10日23:59:59前随时支付569.6元，否则在11日起， 你的服务被中止，并进入**欠款**状态。

***使用定期支付和节省计划***

第一个月，每个小时，你的应计费用是0.6元（0.001元 x 0.6节省系数 x 1000次请求）并通过节省计划进行支付，在下个月1日生成的账单内容如下：

- 账单金额：401.76元（0.001元 x 0.6节省系数 x 300000次请求 + 0.00088元 x 0.6节省系数 x 420000次请求）
- 应付金额：0元
- 账单状态：已结清

此时你不需要任何操作，你的可用额度为100元，剩余的承诺金额为98.24元（500元节省计划 - 401.76元账单金额）。

在第三个月的1日将生成第二个月的账单，账单内容如下：

- 账单金额：607.48元（0.001元 x 0.6节省系数 x 155334次请求 + 0.001元 x 144666次请求 + 0.00088元 x 420000次请求）
- 应付金额：509.24元（614.86元账单金额 - 98.24元节省计划）
- 账单状态：待支付

此时你的可用额度为-409.24元（100元可用额度 - 509.24元应付金额）。你可以在10日23:59:59前随时支付409.24元，否则在11日起，你的服务被中止，并进入**欠款**状态。
...
- **银行转帐：**对于中国地区的企业开发者，我们支持使用银行转账进行支付：
  ...
  - 当完成转账后，你无需任何操作，我们将在收到转账后核对付款信息和金额，之后将转账金额添加到账号可用额度中，你将收到邮件通知。
  - 银行转账以银行到账为准（如遇节假日可能会延后），无法做到实时支付。
