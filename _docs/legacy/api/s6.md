...
**商业版：**
```
https://api.qweather.net/s6/weather/{weather-type}?{parameters}
```

**免费版：**
```
https://free-api.qweather.net/s6/weather/{weather-type}?{parameters}
```
...
```
# 获取北京实况天气
https://api.qweather.net/s6/weather/now?location=beijing&key=xxx
```
...
**商业版：**
```
https://api.qweather.net/s6/weather/{grid-type}?{parameters}
```

**免费版：**
```
https://free-api.qweather.net/s6/weather/{grid-type}?{parameters}
```
...
```
# 获取某一个坐标的格点实况天气
https://api.qweather.net/s6/weather/grid-now?location=116.4,39.9&key=xxx
```
<h3 class="no_toc">请求参数</h3>
...
```
https://api.qweather.net/s6/weather/grid-minute?{parameters}
```
...
```
# 获取某一坐标分钟级降水情况：
https://api.qweather.net/s6/weather/grid-minute?location=116.4,39.9&key=xxx
```
...
**商业版：**
```
https://api.qweather.net/s6/scenic?{parameters}
```
...
```
# 获取北京天坛公园天气情况：
https://api.qweather.net/s6/scenic?location=CN10101010004A&key=xxx
```
...
**商业版：**
```
https://api.qweather.net/s6/weather/historical?{parameters}
```
`parameters`代表请求参数，包括必选和可选参数。所有请求参数均使用`&`进行分隔，参数值存在中文或特殊字符的情况，需要对参数进行 **url encode**。
...
```
# 获取北京的前一天天气数据
https://api.qweather.net/s6/weather/historical?location=CN101010100&date=2017-05-31&key=xxx
```
...
**商业版(指定地区）：**
```
https://api.qweather.net/s6/alarm?{parameters}
```
**商业版(全部地区）：**
```
https://api.qweather.net/s6/alarm/all?{parameters}
```
...
```
# 获取北京的灾害预警信息
https://api.qweather.net/s6/alarm?location=beijing&key=xxx
```
...
**商业版：**
```
https://api.qweather.net/s6/air/{air-type}?{parameters}
```

**免费版：**
```
https://free-api.qweather.net/s6/air/{air-type}?{parameters}
```
...
```
# 获取北京空气质量实况天气：
https://api.qweather.net/s6/air/now?location=beijing&key=xxx
```
...
<h3 class="no_toc">请求URL</h3>
```
https://api.qweather.net/s6/solar/solar-elevation-angle?{parameters}
```
...
```
# 获取某一个坐标的太阳高度
https://api.qweather.net/s6/solar/solar-elevation-angle?lat=39.9&lon=116.4&date=20170809&time=1230&alt=43&tz=8&key=xxx
```
...
**商业版：**
```
https://api.qweather.net/s6/map/cloudmap?{parameters}
```
